...
def test_context():
    CartPoleDQNContextSimple(6000, True).run_episode()
    CartPoleDQNContextSimple(6000, False).run_episode()
...
def time_sgd(n: int):
    context = CartPoleDQNContextSimple(6000, True)
    t1 = datetime.datetime.now()
    for i in range(n):
        context.run_episode()
    t2 = datetime.datetime.now()
    sgd_steps_per_hour = context.num_steps/(t2 - t1).total_seconds()*3600
    print('sgd steps per hour', str(sgd_steps_per_hour))
...
def train_and_save(iterations: int):
    ...
    while context.num_steps < iterations:
        ...
        ep += 1
        if ep % 1000 == 0:
            print(str(context.num_steps) + ' steps complete.')
            torch.save(context.net, './cartpole-net-v2')
    torch.save(context.net, './cartpole-net-v2')

train_and_save(43*70000)
